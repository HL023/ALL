...
function checkHero() {
    ...
    if (!gotHit) {                  // If a RedBall (projectiles) still hasn't hit the Hero
        ...
        if(score < 3000)            // Fast start (the game's really slow at the beginning)
            score += 25;
        else
...
function createProjectile() {
    ...
    if(projectiles.length < (gameSpeed/5)) {      // Number of RedBalls (projectiles) depends on the gameSpeed
        ...
        if(projectileX < projectileY) {
            ...
            if(randomness == 0) {           // Spawns LEFT
                ...
                projectiles.push( { x: 10, 
                                    y: projectileY, 
                                    dx: ((RADIUS+gameSpeed)*Math.cos(angle))/10, 
                                    dy: ((RADIUS+gameSpeed)*Math.sin(angle))/10 
                                } )
            }
            else {                          // Spawns RIGHT
...
function stuffProjectile(projectilePart) {                  // Both draw and Move -> stuff :^)
    ...
    if(projectilePart.x > gameCanvas.width-10 || projectilePart.x < 10){
        projectilePart.dx = -1 * projectilePart.dx;
        if(projectilePart.dx < 0){
            projectilePart.dx += -add_speed;
        }
        else{
            projectilePart.dx += add_speed;
        }
    } 
    if(projectilePart.y > gameCanvas.height-10 || projectilePart.y < 10){
        projectilePart.dy = -1 * projectilePart.dy;
        if(projectilePart.dy < 0){
            projectilePart.dy += -add_speed;
        }
        else{
            projectilePart.dy += add_speed;
        }
...
function moveHero() {
    ...
    if(spacebar){
        ctx.beginPath();
        ctx.arc(hero[0].x, hero[0].y, 10, 0, Math.PI*2);
...
function createCircle() { 
    ...
    if(circles.length < 1 && counter >= 59) {               // No more than 5 because of REASONZ
        counter = 0;
