...
function createProjectile() {
    ...
    if(projectiles.length < 1) {      // Number of RedBalls (projectiles) depends on the gameSpeed
...
function stuffProjectile(projectilePart) {                  // Both draw and Move -> stuff :^)
    ...
    for(var n = 0; n < circles.length; n++) {
        let circ = circles[n];
        let dist = Math.sqrt((circ.x-projectilePart.x)*(circ.x-projectilePart.x) + (circ.y-projectilePart.y)*(circ.y-projectilePart.y)) -circ.r -5;
